// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;

import {MoodNft} from "src/MoodNft.sol";
import {Test, console} from "forge-std/Test.sol";

contract MoodNftTest is Test {
    MoodNft moodNft;
    string public constant HAPPY_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_URI =
        "data:application/json;base64,";
    string public constant HAPPY_SVG_URI =
        "data:application/json;base64,";

    address USER = makeAddr("user");

    /* Modifiers */
    modifier mintMoodNft() {
        vm.prank(USER);
        moodNft.mintNft();
        _;
    }

    function setUp() public {
        moodNft = new MoodNft(SAD_SVG_IMAGE_URI, HAPPY_SVG_IMAGE_URI);
    }

    /*===============================================
                     INIT MOOD          
    ===============================================*/
    function testMoodStateInit() public view {
        // Arrange / Act / Assert
        assertEq(
            keccak256(abi.encodePacked(moodNft.getHappySvgImgUri())), keccak256(abi.encodePacked(HAPPY_SVG_IMAGE_URI))
        );
        assertEq(keccak256(abi.encodePacked(moodNft.getSadSvgImgUri())), keccak256(abi.encodePacked(SAD_SVG_IMAGE_URI)));
        assertEq(moodNft.getTokenCounter(), 0);
    }

    function testViewTokenURI() public mintMoodNft {
        // Arrange / Act / Assert
        console.log(moodNft.tokenURI(0));
        assertEq(keccak256(abi.encodePacked(moodNft.tokenURI(0))), keccak256(abi.encodePacked(HAPPY_SVG_URI)));
    }

    /*===============================================
                     FLIP MOOD          
    ===============================================*/
    function testCanNotFlipMoodIfNotOwner() public mintMoodNft {
        // Arrange
        address notOwner = makeAddr("notOwner");
        // Act / Assert
        vm.prank(notOwner);
        vm.expectRevert(MoodNft.MoodNft__CantFlipMoodIfNotOwner.selector);
        moodNft.flipMood(0);
    }

    function testFirstFlipMoodShouldBeSad() public mintMoodNft {
        // Arrange / Act
        vm.prank(USER);
        moodNft.flipMood(0);
        // Assert
        assertEq(keccak256(abi.encodePacked(moodNft.tokenURI(0))), keccak256(abi.encodePacked(SAD_SVG_URI)));
    }

    function testSecondFlipMoodShouldBeHappy() public mintMoodNft {
        // Arrange
        vm.prank(USER);
        moodNft.flipMood(0);
        // Act
        vm.prank(USER);
        moodNft.flipMood(0);
        // Assert
        assertEq(keccak256(abi.encodePacked(moodNft.tokenURI(0))), keccak256(abi.encodePacked(HAPPY_SVG_URI)));
    }
}
